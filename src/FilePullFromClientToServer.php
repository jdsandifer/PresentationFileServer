...
class FilePullFromClientToServer extends NoMagicFunctions
{
    private $file = null;
    private $client = null;
    private $server = null;
     ...
     */
    public function run() {
        // Run file transfer and record result.
        $transferResult = $this->transferProtocol->pullFile( $this->file, $this->client, $this->server );
        
        // Log results
        if ( $transferResult === "SUCCESS" ) {
            // log file transfer
            $logMessage = "Pulled {$this->file->name()} from {$this->client->room()} {$this->client->type()} via {$this->transferProtocol}.";
            $adminPulled = ( $this->adminName !== null );
            if ( $adminPulled ) {
            ...
            // log error
            $errorMessage = "File pull error: {$transferResult}";
